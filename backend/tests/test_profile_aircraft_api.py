...
def test_attach_adds_the_aircraft_and_returns_the_list(api_client, member, profile, aircraft):
...
def test_attach_is_idempotent(api_client, member, profile, aircraft):
    api_client.force_login(member)
...
def test_attach_creates_the_profile_when_there_is_none(api_client, member, aircraft):
    api_client.force_login(member)
    response = api_client.post(ATTACH_URL, {"aircraft_id": aircraft.id}, format="json")
    assert response.status_code == 200
...
def test_attach_404s_on_an_unknown_aircraft(api_client, member, profile):
    api_client.force_login(member)
    assert api_client.post(ATTACH_URL, {"aircraft_id": 999_999}, format="json").status_code == 404


def test_attach_validates_the_body(api_client, member, profile):
    api_client.force_login(member)
...
def test_attach_only_touches_the_callers_profile(api_client, member, profile, aircraft):
...
def test_detach_removes_the_aircraft(api_client, member, profile, aircraft):
...
def test_detach_is_a_no_op_when_it_was_never_attached(api_client, member, profile, aircraft):
    api_client.force_login(member)
    assert api_client.delete(detach_url(aircraft.id)).status_code == 204


def test_detach_404s_on_an_unknown_aircraft(api_client, member, profile):
    api_client.force_login(member)
    assert api_client.delete(detach_url(999_999)).status_code == 404


def test_detach_leaves_the_aircraft_record_alone(api_client, member, profile, aircraft):
    profile.aircraft.add(aircraft)
    api_client.force_login(member)
...
def test_detach_does_not_affect_another_members_attachment(api_client, member, profile, aircraft):
    stranger = MemberProfileFactory(user=UserFactory(email="stranger@example.test"))
...
def test_several_aircraft_can_be_attached(api_client, member, profile):
...
def test_every_role_may_manage_their_own_aircraft(api_client, all_role_users, aircraft):
