...
def test_save_normalizes_the_n_number() -> None:
    ...
    aircraft = AircraftFactory.create(n_number="  n4 21 cd ")
...
def test_n_number_is_unique_after_normalization() -> None:
    ...
    AircraftFactory.create(n_number="N999ZZ")
...
def test_insurance_is_current_today_counts() -> None:
    ...
    aircraft = AircraftFactory.create(n_number="N1AA", insurance_expiration=today)
...
def test_insurance_expired_yesterday() -> None:
    ...
    aircraft = AircraftFactory.create(
        n_number="N2AA", insurance_expiration=today - timedelta(days=1)
    )
...
def test_missing_insurance_is_not_current() -> None:
    ...
    aircraft = AircraftFactory.create(n_number="N3AA", insurance_expiration=None)
    assert aircraft.insurance_is_current is False
...
def test_str_and_display_name() -> None:
    ...
    aircraft = AircraftFactory.create(n_number="N6AA", make="Piper", model="Archer")
...
def test_str_without_make_or_model() -> None:
    ...
    aircraft = AircraftFactory.create(n_number="N7AA", make="", model="")
