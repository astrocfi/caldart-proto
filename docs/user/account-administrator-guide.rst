...
for a member whose profile nobody has touched since the account was made.  A
payment, a renewal or a membership grant is not an edit to the profile, so the
date answers "how current are these details?" rather than "when did anything
happen to this member?"  Below the strip are four tabs.  Arrow keys move between the tabs, and the tab you are on is part of the
