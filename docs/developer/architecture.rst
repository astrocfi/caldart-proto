...
without joining the barrel: the guards and the route table are its only
callers, and both import it by name.  ``components/useClickOutside.ts`` is the
hook behind that dismissal, for any popover that wants it.
``components/useDebounced.ts`` sits beside the primitives
