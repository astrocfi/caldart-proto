...
The specs live in ``frontend/e2e/``.  They use the **mock** payment provider,
so no keys are needed and no money moves; ``PAYMENTS_MOCK_ENABLED`` must be on,
which it is by default in development.
