...
- Stay within the importing feature (`src/portal/features/<feature>/`) with a relative import (`./form`, `../profile/api`); reach anything outside it — another feature, `src/portal/components/`, `src/portal/api/`, `src/test/` — through the `@/` alias (or `@test/` for the test helpers), never by climbing out with `../..`. ESLint's `no-restricted-imports` enforces both rules.
