...
| System | `make check` | Django system checks (`--fail-level WARNING`), missing-migration check, deployment checks against the production settings (`check --deploy` with the `security`, `caches` and `async_support` tags against `caldart.settings.prod`), production frontend build |
