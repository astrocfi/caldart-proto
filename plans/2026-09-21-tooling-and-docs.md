...
- **Refs:** #61
- **Branch:** `chore/ruff-rule-sets`; database `caldart_ruff_rule_sets`
- **Owns:** `pyproject.toml#ruff`, every backend `.py` file the new rules flag (fixes only), `backend/apps/accounts/seed.py#demo-password-constant`, `.claude/rules/python.md#ruff-categories`, `.claude/rules/python_testing.md#assert`, `docs/developer/testing.rst#ruff`.
- **Steps:**
  1. Enable the eleven rule sets of §5 in one commit that also carries the two documented exceptions (`RUF012` globally, `S101` for tests) and the `ARG` note.
  2. Fix every diagnostic, one commit per rule set, so each is reviewable: the five `N` names, the commented-out code `ERA` finds, the `PT` and `PERF` rewrites, the `S105`/`S106` literals (import the seed's password constant), the two `subprocess` calls, and the rest.
  3. Delete every `noqa` for an unselected rule; `RUF100` proves none remain.
  4. Update the rule table in `python.md` §7 (every set now enabled, the two exceptions, the `ARG` decision), the `assert` note in `python_testing.md`, and the ruff sentence in `testing.rst`.
- **Verify:** `uv run ruff check backend` clean; `uv run ruff check backend --select ARG --statistics | tail -1` still reports the count `python.md` states; `git grep -n "noqa"` lists only lines whose rule is selected and that carry a comment; the full backend suite passes.
